...
const totalAmount = computed(() => {
  return stallOrders.value.reduce((sum, order) => sum + (order.price * order.weight), 0);
});
...
const generateOrderInfo = () => {
  ...
  stallOrders.value.forEach(order => {
    info += `${order.vegetable}：${order.weight}斤 × ${order.price}元/斤 = ${order.price * order.weight}元\n`;
  });
  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="form-item">
        ...
        <select v-model="formData.stallId" class="form-control">
      ...
      <button class="btn btn-primary" @click="generateOrderInfo">生成订单信息</button>
    </div>
    
    <div v-if="orderInfo" class="card" style="margin-top: 16px;">
      <div class="card-header">
        <h4>生成的订单信息</h4>
        <div style="float: right;">
          <button class="btn btn-default" @click="copyOrderInfo">复制</button>
          <button class="btn btn-default" @click="exportOrderInfo">导出</button>
        </div>
      </div>
