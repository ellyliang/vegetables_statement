...
const currentMerchant = ref({ id: '', name: '', address: '', contact: '' });
...
<template>
  <div>
    ...
    <div style="margin-bottom: 16px;">
      <button class="btn btn-primary" @click="openAddDialog">添加大排档</button>
      <button class="btn btn-default" @click="exportMerchants">导出数据</button>
    </div>
    <table class="table">
      <thead>
        <tr>
      ...
      <tbody>
        <tr v-for="merchant in merchants" :key="merchant.id">
          ...
          <td>
            <button class="btn btn-primary" @click="openEditDialog(merchant)">编辑</button>
            <button class="btn btn-default" @click="deleteMerchant(merchant.id)">删除</button>
          </td>
        </tr>
      </tbody>
    </table>
    ...
    <!-- 编辑对话框 -->
    <div v-if="isEditing" style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.5); display: flex; align-items: center; justify-content: center; z-index: 1000;">
      <div style="background: white; padding: 24px; border-radius: 4px; width: 400px;">
        <h4>{{ currentMerchant.id ? '编辑大排档' : '添加大排档' }}</h4>
        <div class="form-item">
          <label class="form-label">名称：</label>
          <input v-model="currentMerchant.name" type="text" class="form-control">
        </div>
        <div class="form-item">
          <label class="form-label">地址：</label>
          <input v-model="currentMerchant.address" type="text" class="form-control">
        </div>
        <div class="form-item">
          <label class="form-label">联系方式：</label>
          <input v-model="currentMerchant.contact" type="text" class="form-control">
        </div>
        <div style="margin-top: 16px; text-align: right;">
          <button class="btn btn-default" @click="closeEditDialog">取消</button>
          <button class="btn btn-primary" @click="saveMerchant">保存</button>
        </div>
      </div>
    </div>
  </div>
